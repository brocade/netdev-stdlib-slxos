...
# File           : puppet/provider/slxos_netdev_lag/slxos.rb
# Version        : 06-12-2017
# Description    : 
#
#   This file contains the SLXOS specific code to implement a 
#   netdev_lag.  This provider will allow you to manage and 
#   create LAG interfaces in SLXOS.
#
...
Puppet::Type.type(:slxos_netdev_lag).provide(:slxos) do
  @doc = "Manage SLXOS Port-Channel interfaces"
 ...
 attr_accessor :name, :description, :mtu, :speed, :lacp, :minimum_links, :links, :type, :target
  ...
  def exists?
	...
	Puppet.debug("Puppet::Device::SLXOS: Po Exists check  #{resource[:name]}.")
  ...
  def create
    notice("#{self.resource.type}: CREATE #{resource[:name]}")
     ...
     Puppet.debug("Puppet::Device::SLXOS: connecting to SLXOS device #{@url.host}, #{@url.user}, #{@url.password}, #{@url.port}.")
